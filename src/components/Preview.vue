...
export default {
    ...
    computed: {
        previewSourceCode() {
            return this.sourceCode.replace(/'\.\.\/\.\.\/index'/g, `'@tencent/my-ui'`)
        }
    },
    async mounted() {
        if (this.compName && this.demoName) {
            if (isDev) {
                ...
                this.sourceCode = await fetch(`${isDev ? '' : '/MY-UI'}/packages/${this.compName}/docs/${this.demoName}.vue`).then((res) => res.text())
            }
        }
        await this.$nextTick()
        Prism.highlightAll()
